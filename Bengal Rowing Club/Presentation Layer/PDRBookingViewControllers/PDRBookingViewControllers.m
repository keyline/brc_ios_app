...
@interface PDRBookingViewControllers ()<ServerConnectionDelegate,SelectDropDownDelegate>{
    SpinnerView *mSpinnerView;
    NSMutableArray* arrPDRList;
...
@property (weak, nonatomic) IBOutlet UITableView *tblVw_PDR;
@property (weak, nonatomic) IBOutlet UIView *vw_Dining;
@end

@implementation PDRBookingViewControllers
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    arrPDRList = [[NSMutableArray alloc] init];
    arrayOfSelectedBoolValues = [[NSMutableArray alloc]init];
    _tblVw_PDR.hidden = YES;
    if ([Utility isNetworkAvailable]) {
        ...
        [mServerConnection pdr:mUser.token];
    }
    else
    {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if(indexPath.section==0){
        ...
        [cell.btn_SendRequest setTitle:@"Book Now" forState:UIControlStateNormal];
...
-(void)PDRByDate{
    if ([Utility isNetworkAvailable]) {
        ...
        ServerConnection *mServerConnection = [[ServerConnection alloc] init];
        mServerConnection.delegate = self;
        [mServerConnection getPdrByDate:mUser.token BookingDate:_txtFld_SelectDate.text];
    }
    else
    {
        [self.view setUserInteractionEnabled:YES];
...
-(void)timeSlotsForTable{
    if ([Utility isNetworkAvailable]) {
        ...
        ServerConnection *mServerConnection = [[ServerConnection alloc] init];
        mServerConnection.delegate = self;
        [mServerConnection getPdrTimeSlots:mUser.token DinningID:strDinnigID BookingDate:_txtFld_SelectDate.text];
    }
    else
    {
        [self.view setUserInteractionEnabled:YES];
...
- (IBAction)click_backPDR:(UIButton *)sender {
...
-(IBAction)click_Submit:(UIButton *)sender{
    ...
    if([DataValidation isNullString:strTimeSlotID]==YES){
        ...
        User *mUser = [User new];
        mUser = [Utility getUserInfo];
        NSDictionary *dic = [NSDictionary dictionaryWithObjectsAndKeys: @"2", @"dining_type",
        ...
        if ([Utility isNetworkAvailable]) {
            ...
            ServerConnection *mServerConnection = [[ServerConnection alloc] init];
            mServerConnection.delegate = self;
            [mServerConnection insert_pdr_booking:jsonString];
        }
        else
        {
            [self.view setUserInteractionEnabled:YES];
...
-(void)dp_Selected {
    //[self timeSlots];
    for (int i = 0; i<[arrPDRList count]; i++) {
        if([[self.downPicker text] isEqualToString:[NSString stringWithFormat:@"%@",[[arrPDRList valueForKey:@"name"] objectAtIndex:i]]]){
            strDinnigID = [NSString stringWithFormat:@"%@",[[arrPDRList valueForKey:@"dining_id"] objectAtIndex:i]];
            break;
        }
    }
    ...
    if([result isKindOfClass:[NSError class]]){
        ...
        else if ([result isKindOfClass:[NSDictionary class]])
        {
            ...
            if([[dict valueForKey:@"status"] integerValue]==1)
            {
                ...
                if([[[dict valueForKey:@"data"] valueForKey:@"pdr"] count]>0){
                    if(arrPDRList.count>0){
                        [arrPDRList removeAllObjects];
                        [arrayOfSelectedBoolValues removeAllObjects];
                    }
                    for (int i = 0; i<[[[dict valueForKey:@"data"] valueForKey:@"pdr"] count]; i++) {
                        [arrPDRList addObject:[[[dict valueForKey:@"data"] valueForKey:@"pdr"] objectAtIndex:i]];
                    }
                    _tblVw_PDR.hidden = YES;
                    [_tblVw_PDR reloadData];
                    _txtFld_SelectDinning.text = @"Select Dinning Area";
                    self.downPicker = [[DownPicker alloc] initWithTextField:_txtFld_SelectDinning withData:[arrPDRList valueForKey:@"name"]];
