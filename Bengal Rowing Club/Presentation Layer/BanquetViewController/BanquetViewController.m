...
@interface BanquetViewController ()<ServerConnectionDelegate,SelectDropDownDelegate>{
    SpinnerView *mSpinnerView;
    NSMutableArray* arrBanquetList;
...
@property (weak, nonatomic) IBOutlet UITableView *tblVw_Banquet;
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    arrBanquetList = [[NSMutableArray alloc] init];
    arrayOfSelectedBoolValues = [[NSMutableArray alloc]init];
    _tblVw_Banquet.hidden = YES;
    if ([Utility isNetworkAvailable]) {
        ...
        [mServerConnection banquet:mUser.token];
    }
    else
    {
...
- (IBAction)click_backBanquet:(UIButton *)sender {
...
-(IBAction)click_Submit:(UIButton *)sender{
    ...
    if([DataValidation isNullString:strTimeSlotID]==YES){
        ...
        User *mUser = [User new];
        mUser = [Utility getUserInfo];
        NSDictionary *dic = [NSDictionary dictionaryWithObjectsAndKeys: @"1", @"dining_type",
        ...
        if ([Utility isNetworkAvailable]) {
            ...
            ServerConnection *mServerConnection = [[ServerConnection alloc] init];
            mServerConnection.delegate = self;
            [mServerConnection insert_booking:jsonString];
        }
        else
        {
            [self.view setUserInteractionEnabled:YES];
...
-(void)timeSlots{
    if ([DataValidation isNullString:_txtFld_SelectDate.text]==YES || [DataValidation isNullString:_txtFld_SelectDinning.text]==YES) {
        ...
        if ([Utility isNetworkAvailable]) {
            ...
            ServerConnection *mServerConnection = [[ServerConnection alloc] init];
            mServerConnection.delegate = self;
            for (int i = 0; i<[arrBanquetList count]; i++) {
                if([[self.downPicker text] isEqualToString:[NSString stringWithFormat:@"%@",[[arrBanquetList valueForKey:@"name"] objectAtIndex:i]]]){
                    strDinnigID = [NSString stringWithFormat:@"%@",[[arrBanquetList valueForKey:@"dining_id"] objectAtIndex:i]];
                    break;
                }
            }
            [mServerConnection getTimeSlots:mUser.token DinningID:strDinnigID BookingDate:_txtFld_SelectDate.text];
        }
        else
        {
            [self.view setUserInteractionEnabled:YES];
    ...
    if([result isKindOfClass:[NSError class]]){
        ...
        else if ([result isKindOfClass:[NSDictionary class]])
        {
            ...
            if([[dict valueForKey:@"status"] integerValue]==1)
            {
                ...
                if([[[dict valueForKey:@"data"] valueForKey:@"banquets"] count]>0){
                    for (int i = 0; i<[[[dict valueForKey:@"data"] valueForKey:@"banquets"] count]; i++) {
                        [arrBanquetList addObject:[[[dict valueForKey:@"data"] valueForKey:@"banquets"] objectAtIndex:i]];
                    }
                    self.downPicker = [[DownPicker alloc] initWithTextField:_txtFld_SelectDinning withData:[arrBanquetList valueForKey:@"name"]];
        ...
        if([[dict valueForKey:@"status"] integerValue]==1)
        {
            if([[dict valueForKey:@"is_timeslot"] integerValue]==1){
                if([[dict valueForKey:@"data"] valueForKey:@"timeslots"]!=nil){
                    ...
                    strMG_banquet = [dict valueForKey:@"mg_banquet"];
                }
                else{
                }
                _tblVw_Banquet.hidden = NO;
                [_tblVw_Banquet reloadData];
            }
            else{
                UIAlertController* alert = [UIAlertController alertControllerWithTitle:ALERT_TITLE message:[NSString stringWithFormat:@"%@",[dict valueForKey:@"message"]] preferredStyle:UIAlertControllerStyleAlert];
