//
//  MyPDRBookingViewController.m
//  Bengal Rowing Club
//
//  Created by Chakraborty, Sayan on 30/11/21.
...
@interface MyPDRBookingViewController ()<ServerConnectionDelegate>{
    SpinnerView *mSpinnerView;
    NSMutableArray *arrMyPDRBookingList;
    NSInteger index;
}
@property (weak, nonatomic) IBOutlet UITableView *tblVw_PDRBooking;
@end

@implementation MyPDRBookingViewController
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    arrMyPDRBookingList = [[NSMutableArray alloc] init];
    _tblVw_PDRBooking.hidden = YES;
    if ([Utility isNetworkAvailable]) {
        ...
        [mServerConnection myPDRBooking:mUser.token MembershipNo:mUser.user_membership_no];
    }
    else
    {
...
{
    return arrMyPDRBookingList.count;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    cell.lbl_BookingID.text = [NSString stringWithFormat:@"%@",[[arrMyPDRBookingList valueForKey:@"booking_id"] objectAtIndex:indexPath.row]];
    cell.lbl_DinningArea.text = [NSString stringWithFormat:@"%@",[[arrMyPDRBookingList valueForKey:@"dining_name"] objectAtIndex:indexPath.row]];
    cell.lbl_TimeSlot.text = [NSString stringWithFormat:@"%@",[[arrMyPDRBookingList valueForKey:@"timeslot_name"] objectAtIndex:indexPath.row]];
    cell.lbl_BookingDate.text = [NSString stringWithFormat:@"%@",[[arrMyPDRBookingList valueForKey:@"booking_date"] objectAtIndex:indexPath.row]];
    cell.lbl_TimePeriod.text = [NSString stringWithFormat:@"%@",[[arrMyPDRBookingList valueForKey:@"time_period"] objectAtIndex:indexPath.row]];
    if([DataValidation isNullString:[[arrMyPDRBookingList valueForKey:@"cancelled_by"] objectAtIndex:indexPath.row]]==YES){
...
#pragma mark - BACK FROM PDR

- (IBAction)click_BackPDRBooking:(UIButton *)sender {
...
-(IBAction)click_Cancel:(UIButton *)sender{
    if ([Utility isNetworkAvailable]) {
        ...
        mServerConnection.delegate = self;
        index = sender.tag;
        [mServerConnection cancel_pdr_booking:mUser.token MembershipNo:mUser.user_membership_no BookingID:[[arrMyPDRBookingList valueForKey:@"booking_id"] objectAtIndex:sender.tag]];
    }
    else
    {
        [self.view setUserInteractionEnabled:YES];
    ...
    else if ([result isKindOfClass:[NSDictionary class]])
    {
        ...
        if([[dict valueForKey:@"status"] integerValue]==1)
        {
            ...
            if([[[dict valueForKey:@"data"] valueForKey:@"is_my_booking"] integerValue]==1){
                if([[dict valueForKey:@"data"] valueForKey:@"my_pdr_booking"]!=nil){
                    if(arrMyPDRBookingList.count>0){
                        [arrMyPDRBookingList removeAllObjects];
                    }
                    for (int i = 0; i<[[[dict valueForKey:@"data"] valueForKey:@"my_pdr_booking"] count]; i++) {
                        [arrMyPDRBookingList addObject:[[[dict valueForKey:@"data"] valueForKey:@"my_pdr_booking"] objectAtIndex:i]];
                    }
                    NSLog(@"PDR Booking: %@",arrMyPDRBookingList);
                    _tblVw_PDRBooking.hidden = NO;
                    [_tblVw_PDRBooking reloadData];
                }
                else{
                }
            }
    ...
    else if ([result isKindOfClass:[NSDictionary class]])
    {
        ...
        if([[dict valueForKey:@"status"] integerValue]==1)
        {
            ...
            if([[[dict valueForKey:@"data"] valueForKey:@"is_booking_cancel"] integerValue]==1){
                ...
                [alert addAction:defaultAction];
                [self presentViewController:alert animated:YES completion:nil];
                NSMutableDictionary *dict = [arrMyPDRBookingList[index] mutableCopy];
                [dict setObject:@"member" forKey:@"cancelled_by"];
                [arrMyPDRBookingList replaceObjectAtIndex:index withObject:dict];
                [_tblVw_PDRBooking reloadData];
            }
            else{
                UIAlertController* alert = [UIAlertController alertControllerWithTitle:ALERT_TITLE message:[NSString stringWithFormat:@"%@",[dict valueForKey:@"message"]] preferredStyle:UIAlertControllerStyleAlert];
